...
%% A4 = 21.0cm ⨉ 29.7cm
\usepackage[xetex,a4paper,inner=1.5cm,outer=1cm,top=2cm,bottom=1.5cm]{geometry}
{{KONEC POKUD}}
{{POKUD JE FORMÁT pdf-b5}}

%% B5 = 17.6cm ⨉ 25.0cm
\usepackage[xetex,b5paper,inner=1.5cm,outer=1cm,top=1.75cm,bottom=1.25cm]{geometry}
{{KONEC POKUD}}
...
\newcommand*{\kapitola}[2][]{%
    \clearpage\renewcommand*{\nazevkapitoly}{#2}\chapter{#2}%
}
...
\newcommand*{\klavesa}[1]{%
    \tcbox[size=fbox,on line,leftrule=0.4pt,toprule=0.4pt,rightrule=1.5pt,bottomrule=1.5pt,boxsep=1.5pt,left=1.5pt,right=1.5pt]{{\sffamily #1}}%
    }%
...
% #1 = nastavení rodiny písma (např. \lmmathfamily) + znak ikony
\newcommand*{\ikonazaklinadla}[1]{{%
    \setlength{\unitlength}{1em}%
    \begin{picture}(1,0.5)%
        %\put(0.53,0.35){\circle{1.2}}%
        \put(0.5,0){\makebox[0pt]{{#1}}}%
    \end{picture}%
}}
...
\newcommand{\zaklinadlo}[6][]{{%
    ...
    \newcommand*{\zacatek}[1]{%
        \noindent\begin{tabular}{%
            % číslo zaklínadla:
            >{\raggedright##1\fontsize{9pt}{0pt}\selectfont}p{\sirkacislazaklinadla}@{}%
            % ikona zaklínadla:
            >{\centering##1}p{\sirkaikony}@{\hspace{2pt}}%
            % titulek zaklínadla + mezera před řádky zaklínadla:
            >{\raggedright##1\fontsize{10pt}{10pt}\selectfont}p{\sirkatitulkuzaklinadla}%
            @{\hspace{10pt}}%
            % řádky zaklínadla:
            |>{\raggedright\zaklinadlofamily\zaklinadlosize}p{\sirkaradkuzaklinadla}%
        }%
    }%
    \newcommand*{\konec}{\tabularnewline\end{tabular}}%
    \ifnumodd{#2}{%
